...
class Informativeness():
    ...
    def __init__(self, mode, model_path, filter=None, vocab_path=None,
        ...
        if self._mode == 'cbow':
            if not model_path:
                raise Exception('Unspecified w2v model path for '
                                'informativeness in CBOW mode')
    ...
    def _get_cbow_ctx_shannon_entropy(self, tokens, word_index):
        ...
        shannon_entropy = scipy.stats.entropy(probs)
        print(len(self._model.wv.vocab))
        print(len(probs))
        s2w = 1 - (shannon_entropy / numpy.log(len(self._model.wv.vocab)))
        return s2w
