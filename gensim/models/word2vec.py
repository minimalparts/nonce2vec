...
"""
The original gensim code tries to use the C libraries here.
Nonce2Vec does not have a C implementation, so python is used.
"""
...
def train_sg_pair(model, word, context_index, alpha, nonce_count, learn_vectors=True, learn_hidden=True,
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, sentences=None, size=100, alpha=0.025, window=5, window_decay=0, min_count=5,
            max_vocab_size=None, sample=1e-3, sample_decay=1.0, seed=1, workers=3, min_alpha=0.0001,
    ...
    def finalize_vocab(self, pre_exist_words, update=False):
        ...
        if not update:
            ...
            self.update_weights(pre_exist_words)
    ...
    def update_weights(self, pre_exist_words):
        """
        ...
        newsyn0 = zeros((gained_vocab, self.vector_size), dtype=REAL)
